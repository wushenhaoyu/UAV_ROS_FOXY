  ...
  lslidar_msgs__msg__LslidarPoint__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lslidar_msgs__msg__LslidarPoint * data =
      (lslidar_msgs__msg__LslidarPoint *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lslidar_msgs__msg__LslidarPoint__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          lslidar_msgs__msg__LslidarPoint__fini(&data[i]);
        }
        free(data);
        return false;
      }
